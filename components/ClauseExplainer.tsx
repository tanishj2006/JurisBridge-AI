...
export default function ClauseExplainer({ clause, isHighlighted }: ClauseExplainerProps) {
  ...
  let riskBadge = (
    <span className="px-2.5 py-1 rounded-md text-xs font-bold bg-emerald-950/80 text-emerald-300 border border-emerald-700/60 flex items-center gap-1">
  ...
  if (clause.riskLevel === 'HIGH') {
    riskBadge = (
      <span className="px-2.5 py-1 rounded-md text-xs font-bold bg-rose-950/80 text-rose-300 border border-rose-700/60 flex items-center gap-1">
    ...
    riskBadge = (
      <span className="px-2.5 py-1 rounded-md text-xs font-bold bg-amber-950/80 text-amber-300 border border-amber-700/60 flex items-center gap-1">
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 bg-slate-950/60 p-4 rounded-xl border border-slate-800/80">
        {viewMode === 'plain' ? (
          <div>
            ...
            <p className="text-sm text-slate-200 leading-relaxed">{clause.simplifiedText}</p>
          </div>
        ) : (
          <div>
      ...
      {/* Explanation Callout */}
      <div className="mt-4 bg-slate-900/90 p-4 rounded-xl border border-slate-800">
        ...
        <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{clause.explanation}</p>
      </div>
      ...
      {clause.suggestedRevision && (
        <div className="mt-4 border border-indigo-900/50 rounded-xl overflow-hidden bg-slate-950/80">
          ...
          {isRevisionOpen && (
            <div id={`revision-${clause.id}`} className="p-4 border-t border-indigo-900/40 bg-indigo-950/20 space-y-2">
